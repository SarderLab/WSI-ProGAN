...
#----------------------------------------------------------------------------
# Dataset class that loads data from WSI files.
...
class WSIDataset:
    def __init__(self,
        ...
        tfr_lods = [self.resolution_log2 - i for i in range(2,self.resolution_log2+1)]
        tfr_shapes = []
        for lod in tfr_lods:
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            for tfr_shape, tfr_lod in zip(tfr_shapes, tfr_lods):
                ...
                print('lod: {} | patch_size: {} | downsample: {}'.format(tfr_lod, output_size, this_downsample))

                path_ds = tf.data.Dataset.from_tensor_slices(wsi_files)
                path_ds = path_ds.shuffle(100)
                dset = path_ds.map(lambda filename: tf.py_function(get_random_wsi_batch, [filename,output_size,False,this_downsample], tf.float32), num_parallel_calls=num_threads)
...
def load_dataset(class_name='dataset.WSIDataset', data_dir=None, verbose=False, **kwargs):
